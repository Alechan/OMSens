...
class TestCurvi(unittest.TestCase):
    ...
    # Tests:
    def test_curvi_finds_global_minimum(self):
        curvi_mod = self.tryToImportCurviModule()
        # Get base args for an example function
        x0, obj_func, epsilon, lower_bounds, upper_bounds  = baseLocalGlobalFuncArgsForVectorOf2()
        # Call curvi
        x_opt,f_opt = curvi_mod.curvifgr_simplified(x0,obj_func,lower_bounds,upper_bounds,epsilon)
...
#            self.fail(error_msg)
    ...
    def test_curvi_epsilon_works(self):
        ...
        x_opt_permissive,f_opt_permissive = curvi_mod.curvifgr_simplified(x0,obj_func,lower_bounds,upper_bounds,epsilon_permissive)
        # Call curvi with a MORE strict epsilon
        epsilon_strict = 0.00001
        x_opt_strict,f_opt_strict = curvi_mod.curvifgr_simplified(x0,obj_func,lower_bounds,upper_bounds,epsilon_strict)
    ...
    def tryToImportCurviModule(self):
        try:
            # import curvifgr_simplified as curvifgr_simplified
            import fortran_interface_curvifgr.curvifgr_simplified as curvifgr_simplified
        except:
            error_msg = "The curvi optimizer was not installed correctly and is unavailable"
            self.fail(error_msg)
        return curvifgr_simplified
...
def baseLocalGlobalFuncArgsForVectorOf2():
    x0 = numpy.array([1.74,0.87])
    obj_func = lambda x: 2*x[0]**2-1.05*(x[0]**4)+(1/6)*(x[0]**6)-x[0]*x[1]+x[1]**2
    epsilon = 0.0001
    lower_bounds = 2*[-2]
    upper_bounds = 2*[2]
    return x0, obj_func, epsilon, lower_bounds, upper_bounds
...
# f=2.d0*x(1)**2-1.05d0*x(1)**4+(1.d0/6.d0)*x(1)**6-x(1)*x(2)+x(2)**2